...
export const updateShift = (id: number, body: Partial<{ name: string; startTime: string; endTime: string; crossMidnight: boolean }>): Promise<Shift> =>
...
// Employees
export const getEmployees = (params?: { active?: boolean; shiftId?: number; search?: string }): Promise<Employee[]> =>
  client.get<Employee[]>('/config/employees', { params }).then(r => r.data);

export const updateEmployee = (id: number, body: { shiftId?: number | null; active?: boolean }): Promise<Employee> =>
  client.put<Employee>(`/config/employees/${id}`, body).then(r => r.data);

export const bulkAssignShift = (employeeIds: number[], shiftId: number): Promise<void> =>
  client.post('/config/employees/bulk-assign', { employeeIds, shiftId }).then(() => undefined);

export const deactivateEmployee = (id: number): Promise<void> =>
